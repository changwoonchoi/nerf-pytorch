...
class ScannetDataset(NerfDataset):
    def __init__(self, basedir, **kwargs):
        ...
        if self.split == "train":
            self.skip = 1
        self.test_skip = kwargs.get("test_skip", 10)
        ...
        if self.split == "train":
            index_list_tmp = [i * self.test_skip + j + 1 for i in range(total_dataset_len // self.test_skip + 1) for j in range(self.test_skip - 1)]
            self.index_list = [i for i in index_list_tmp if i < total_dataset_len]
        elif self.split in ["val", "test"]:
            index_list_tmp = [i * self.test_skip for i in range(total_dataset_len // self.test_skip + 1)]
            self.index_list = [i for i in index_list_tmp if i < total_dataset_len]
